...
This is a port to the Zig build system. My C examples work. Shall check with Zig.

uvgRTP is an _Real-Time Transport Protocol (RTP)_ library written in C++ with a focus on simple to use and high-efficiency media delivery over the Internet. It features an intuitive and easy-to-use _Application Programming Interface (API)_, built-in support for transporting _Versatile Video Coding (VVC)_, _High Efficiency Video Coding (HEVC)_, _Advanced Video Coding (AVC)_ encoded video and Opus encoded audio. Furthermore, uvgRTP can be used to transport _Visual Volumetric Video-based Coding (V3C)_ encoded volumetric video. uvgRTP also supports _End-to-End Encrypted (E2EE)_ media delivery using the combination of _Secure RTP (SRTP)_ and ZRTP. uvgRTP has been designed to minimize memory operations to reduce its CPU usage and latency.
